...
screen_size = (900, 600)    #dimensions de la fenetre (largeur et hauteur)
...
scale = 0.1 #facteur permettant de redimensionner l'image
...
while running:  #Tant que l'utilisateur n'a pas ferme la fenetre
    for event in pygame.event.get():    #Gestion des evenements   
        if event.type == pygame.QUIT:   #Si l'utilisateur clique sur le bouton pour fermer la fenetre
            ...
            if image_rect.collidepoint(event.pos[0], event.pos[1]):
                print("Vous avez cliqué sur l'image!")
    
    screen.fill(BG_COLOR)   #D'abord l'arriere plan

    screen.blit(image, image_rect)   #afficher l'image
